	...
	$(document).ready(function(){
		...
		if ($('.isotope-container').length>0) {
			$(window).load(function() {
				...
				var filter = ["","","","",""];//存放4个下拉列表的筛选条件
				$(".filters select").on("change", function ()
				{
				    var index=$(".filters select").index($(this));//第几个下拉列表
				    filter[index] = $(this).val();//获取下拉列表当前值
				    var filterValue=filter[0] + filter[1] + filter[2] + filter[3] +filter[4];//串联4个下拉列表当前值
				    $container.isotope({ filter: filterValue });//筛选
				});
